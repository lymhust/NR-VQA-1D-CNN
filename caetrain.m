function cae = caetrain(cae, FPPG, TPPG, opts)
    cae.rL = [];
    for m = 1 : opts.rounds
        tic;
        disp([num2str(m) '/' num2str(opts.rounds) ' rounds']);
        i1 = randi(numel(FPPG));
        i2  = randi(size(FPPG{i1}{1},2) - opts.batchsize);
        x1{1} = FPPG{i1}{1}(:, i2 : i2 + opts.batchsize - 1);
        x2{1} = TPPG{i1}{1}(:, i2 : i2 + opts.batchsize - 1);
